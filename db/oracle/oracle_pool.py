...
from db.oracle.base_pyoracle_pool import BasePyOraclePool
from config import dbInfo


class OraclePool(BasePyOraclePool):
    """
    ...
    def __init__(self, conf_name=None):
        ...
        """
        self.conf = dbInfo.get("oracle")
        if self.conf is None:
            raise Exception("oracle配置不存在")

        print(self.conf)
        super(OraclePool, self).__init__(**self.conf)
        self.__pool = self.__get_pool(self.user, self.password, self.host, self.port, charset=self.charset,
                                      sid=self.sid, service_name=self.dbname)

    @staticmethod
    def __get_pool(username, password, host, port, sid, charset, service_name=None, pool_size=5):
        """
        ---------------------------------------------
        以下设置，根据需要进行配置
        max                 最大连接数
