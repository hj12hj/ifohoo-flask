...
class ConfigSql:
    ...
    def get_config_list(self, query_data):
        ...
        totle, data = self.db.query_page(
            "select * from dynamic_config where (form_name like :1 or :2 is null) and (form_code = :3 or :4 is null)",
    ...
    def update_config_info(self, config_data):
        staffId = local_token.token_info.get("staffId")
        self.db.execute_sql(
            "update dynamic_config set  form_default_content=:1 , form_name = :2,form_detail_content=:3,update_by=:4 where form_code = :5;",
